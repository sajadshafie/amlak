...
export const getStaticPaths: GetStaticPaths = async () => {
  const res = await fetch(
    `http://api2.talaremelk.ir/Api/Advertisement/Get?pageSize=10`
  );
  const data = await res.json();
  const paths = data.map((v: adviserType) => {
    return {
      params: {
        slug: v.id,
      },
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const res = await fetch(
    `http://api2.talaremelk.ir/Api/Advertisement/Get?id=${params?.slug}`
  );
  const data = await res.json();
  return {
    props: {
